...
export default function SignIn({ fallback = '/studio/portal/' }: { fallback?: string }) {
  ...
  if (!accountsEnabled) {
    return (
      <Shell>
        <h1 className="text-title font-black tracking-display text-ink">Accounts are not on yet</h1>
        ...
        <a href="/library/" className="mt-6 inline-flex rounded-lg bg-ink px-5 py-3 text-sm font-bold text-paper no-underline">
          Look at the library
        </a>
      </Shell>
    )
  ...
  if (checkEmail) {
    return (
      <Shell>
        <h1 className="text-title font-black tracking-display text-ink">Check your email</h1>
        <p className="mt-3 text-[15px] leading-relaxed text-ink-soft">
        ...
        <button
          ...
          className="mt-6 text-sm font-bold text-ink-soft underline underline-offset-4 hover:text-ink"
        >
          Back to signing in
        </button>
      </Shell>
  ...
  return (
    <Shell>
      ...
      <button
        ...
        onClick={google}
        disabled={busy !== null}
        className="mt-7 flex w-full items-center justify-center gap-3 rounded-lg border border-ink-hair bg-surface px-5 py-3.5 text-sm font-bold text-ink transition-colors hover:bg-paper disabled:opacity-50"
      >
        <GoogleMark />
      ...
      <form onSubmit={submit} noValidate>
        <label className="block">
          ...
          <input
            ...
            className="mt-1.5 w-full rounded-lg border border-ink-hair bg-surface px-4 py-3 text-[15px] text-ink outline-none transition-colors placeholder:text-ink-faint focus:border-ink/30"
          />
        </label>
        ...
        {error && (
          <p role="alert" className="mt-4 rounded-lg bg-[#E2473B]/10 px-4 py-3 text-[13px] leading-relaxed text-ink">
            {error}
          </p>
        )}
        ...
        <button
          type="submit"
          disabled={busy !== null}
          className="mt-5 w-full rounded-lg bg-ink px-5 py-3.5 text-sm font-bold text-paper transition-opacity hover:opacity-90 disabled:opacity-50"
        >
      ...
      <p className="mt-8 border-t border-ink-hair pt-5 text-center text-[12px] leading-relaxed text-ink-faint">
        Your systems are yours. They stay on your own machine as you work and follow you to any
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="mx-auto w-full max-w-[26rem] rounded-2xl border border-ink-hair bg-paper p-8 shadow-lift sm:p-10">
      {children}
    </div>
  )
}
